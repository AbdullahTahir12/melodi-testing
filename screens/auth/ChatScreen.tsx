...
const ChatScreen = ({route}) => {
  ...
  // useEffect(() => {
  //   // Retrieve stored messages and show only the first 15 on component mount
  //   const getStoredMessages = async () => {
  //     try {
  //       const storedMessages = await AsyncStorage.getItem('chatMessages');
  //       if (storedMessages) {
  //         const parsedMessages = JSON.parse(storedMessages);
  //         // Limit the displayed messages to the first 15 (inclusive)
  //         const limitedMessages = parsedMessages.slice(0, 15); // Start from index 0 (first)
  //         setMessages(limitedMessages);
  //         setConversation(parsedMessages); // Update conversation history for API payload
  //       }
  //     } catch (error) {
  ...
  useEffect(() => {
    const getStoredMessages = async () => {
      try {
        ...
        if (storedMessages) {
          const parsedMessages = JSON.parse(storedMessages);
          const limitedMessages = parsedMessages.slice(0, 15);
          setMessages(limitedMessages);
  ...
  return (
    <ImageBackground
      ...
      style={styles.backgroundImage}>
        ...
        <Image
          source={require('../assets/girl.jpg')} // Replace with your image path
